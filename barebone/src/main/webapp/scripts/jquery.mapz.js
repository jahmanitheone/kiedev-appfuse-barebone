...
(function( $ ){
  ...
  $.fn.mapz = function(options) {
		...
		if(settings.zoom) {
			...
			// TEAM - BHickey - Add slider controls to the map.
			$("#sliderplus").click(function() {
				zoom('in');
		  		valore = parseInt($("#slider").slider('value')) + 1;
		  		$("#slider").slider('value', valore);

			});
			$("#sliderminus").click(function() {
				// KNOWN ISSUE - NEED TO ENSURE THE VALORE IS NOT BELOW THE MIN SLIDER VALUE BEFORE SLIDING AND ZOOMING OUT.
				zoom('out');
		  		valore = parseInt($("#slider").slider('value')) - 1;
		  		$("#slider").slider('value', valore);
			});
			$("#slider").slider({value:0, min: 0, max: 3, step: 1});
